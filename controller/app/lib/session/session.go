...
func GetManagerConfig() *SessionManagerConfig {
	confFile, err := ioutil.ReadFile(env.SessionConfig)
	if err != nil {
		log.Println("Error while reading session config from " + env.SessionConfig)
		return nil
	}
	...
	if err := json.Unmarshal([]byte(confFile), &config); err != nil {
		log.Println("GetManagerConfig : ", err.Error())
		return nil
	}
...
func (sm *SessionManager) GlobalCleaner() {
	...
	sm.lock.RUnlock()
	log.Println("GlobalCleaner ::: Cleaned stale sessions")
